...
import './commands';
import { IS_CANVAS, IS_RULES, IS_TOC, IS_ALLOWED_ELEMENTS } from './selectors';
import { IS_DATA } from './testdata';

// =============================================================================
// Instruction for Student (FIXED TOC page) — Deep Component Test
//
// QC authority : Author Test Cases -  Instruction for Student.pdf (TC_01–TC_19)
// Source authority:
//   index.html               (TOC entry "Instructions for Students" + #desktop_view)
//   editor/ngcontroller.js   (pageType "studentInstructions" handling)
//   config/config.js         (drop-permission matrix: studentInstructions)
//
// Instructions for Students is a FIXED page in the Table of Contents — it is NOT
// dropped. Tests NAVIGATE to it (cy.navigateToCanvasPage → clicks the
// "Instructions for Students" TOC entry) and author its content / add elements.
// config.studentInstructions permits ONLY header, paragraph (Text), poetry,
// image, tablesNew (Table) drops.
//
...
// See INSTRUCTION-STUDENT-DEEP-AUTOMATION.md.
//
...
// "instructionsstudents" is a logical slug; the verified page identifier is the
// AngularJS pageType "studentInstructions".
// =============================================================================
...
beforeEach(function () {
  ...
  cy.ensureInstructionStudentDeepSetup({ through: 'content', skipPageNav: false });
});

// ===========================================================================
// Suite A — Page reachable in TOC / editor (TC_01–TC_03 family)
// ===========================================================================
describe('IS-A — Instruction for Student: Page reachable in TOC', () => {
  it('TC_NAV — Instructions for Students page is reachable from the TOC', function () {
    skipIfNoUrl(this);
    cy.ensureInstructionStudentDeepSetup({ through: 'page', skipPageNav: false });
    cy.get(IS_CANVAS.desktop, { timeout: 20000 }).should('exist');
    cy.contains(IS_TOC.entryText, { timeout: 15000 }).should('exist');
    cy.log(`✅ TC_NAV: "${IS_RULES.tocLabel}" page reachable (pageType=${IS_RULES.pageType})`);
  });

  it('TC_01/TC_02/TC_03 — Authoring shell + Instructions page load (bootstrap)', function () {
    skipIfNoUrl(this);
    cy.get(IS_CANVAS.desktop).should('exist');
    cy.get(IS_TOC.tocPanel).should('exist');
...
// ===========================================================================
describe('IS-B — Instruction for Student: Content area editable (TC_04–TC_08)', () => {
  it('TC_CONTENT — Instruction content area accepts authored text', function () {
    skipIfNoUrl(this);
    cy.get(IS_CANVAS.paragraphBody).first().setContentEditableOn(IS_DATA.shortInstruction);
    cy.get(IS_CANVAS.paragraphBody).first().should('contain.text', IS_DATA.shortInstruction);
  ...
  it('TC_05 — Body preserves a line break entered in edit mode', function () {
    skipIfNoUrl(this);
    cy.get(IS_CANVAS.paragraphBody).first().then(($el) => {
      const el = $el[0];
      el.focus();
      el.innerHTML = 'First line.<br>Second line.';
      el.dispatchEvent(new Event('input', { bubbles: true }));
    });
    cy.get(IS_CANVAS.paragraphBody).first().find('br').should('exist');
  ...
  it('TC_04/TC_06/TC_07/TC_08 — Rich-text formatting toolbar is reachable on focus', function () {
    skipIfNoUrl(this);
    cy.get(IS_CANVAS.paragraphBody).first().click({ force: true });
...
// ===========================================================================
describe('IS-C — Instruction for Student: Add elements to page (TC_09–TC_12)', () => {
  it('TC_ADD — A Text element can be authored onto the Instructions page', function () {
    skipIfNoUrl(this);
    cy.ensureInstructionStudentDeepSetup({ through: 'element', skipPageNav: false });
    cy.get(IS_CANVAS.sdItem, { timeout: 20000 }).should('have.length.greaterThan', 0);
    cy.log('✅ TC_ADD: Element added to the Instructions-for-Students page canvas');
  });

  it('TC_09/TC_10/TC_11/TC_12 — Page permits the source-allowed element drops', function () {
    skipIfNoUrl(this);
    // config.studentInstructions === true only for header, paragraph, poetry,
    ...
    cy.get(IS_CANVAS.sdItem).should('have.length.greaterThan', 0);
    const allowed = Object.values(IS_ALLOWED_ELEMENTS).map((e) => e.dataType);
...
// ===========================================================================
describe('IS-D — Instruction for Student: Edge cases (TC_17, TC_19)', () => {
  it('TC_17 — Very long instruction content is accepted without crash', function () {
    skipIfNoUrl(this);
    cy.get(IS_CANVAS.paragraphBody).first().setContentEditableOn(IS_DATA.longContent);
    cy.get(IS_CANVAS.paragraphBody).first().invoke('text').then((text) => {
  ...
  it('TC_19 — Emojis / symbols / foreign characters are accepted', function () {
    skipIfNoUrl(this);
    cy.get(IS_CANVAS.paragraphBody).first().setContentEditableOn(IS_DATA.specialCharacters);
    cy.get(IS_CANVAS.paragraphBody).first().should('contain.text', '你好');
...
// Suite E — Security
// ===========================================================================
describe('IS-E — Instruction for Student: Security', () => {
  it('SEC — Script tags in instruction content are not rendered as live <script>', function () {
    skipIfNoUrl(this);
    cy.get(IS_CANVAS.paragraphBody).first().setContentEditableOn(IS_DATA.scriptInjection);
    cy.get(IS_CANVAS.paragraphBody).first().should('not.contain.html', '<script>');
    cy.window().then((win) => {
...
// ===========================================================================
describe('IS-Z — Instruction for Student: Deferred (Bootstrap / Preview / Reader / Publish / lifecycle)', () => {
