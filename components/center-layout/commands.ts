...
// =============================================================================
// Center Layout (layout-2) component commands — reusable workflows.
//
...
//   cy.captureLiveStep
//
// IMPORTANT — Center is a LAYOUT, not an assessment widget:
//   "layout-2" is NOT in support/module-setup.ts ASSESSMENT_DATA_TYPES, so the
...
//   cy.dropComponent('Center'), NOT into a Generic Step column.
// =============================================================================
...
declare global {
  ...
  namespace Cypress {
    interface Chainable<Subject = any> {
      ensureCenterLayoutDeepSetup(options?: {
        through?: 'canvas' | 'dropped';
        skipCanvasNav?: boolean;
      }): Chainable<void>;
      dropCenterLayoutIntoCanvas(): Chainable<void>;
      openCenterLayoutSettings(): Chainable<void>;
    }
  }
}
...
Cypress.Commands.add('ensureCenterLayoutDeepSetup', (options = {}) => {
  ...
  cy.dropCenterLayoutIntoCanvas();
});

Cypress.Commands.add('dropCenterLayoutIntoCanvas', () => {
  cy.navigateToCanvasPage();

  cy.get('#desktop_view').then(($desktop) => {
    const alreadyDropped = $desktop.find(`.sd-item ${LAYOUT_CANVAS.widget}`).length > 0;
    if (alreadyDropped) {
      cy.log('ℹ Center layout already present on canvas');
      return;
    }
  ...
  cy.captureLiveStep('widget-dropped-center-layout');
});

Cypress.Commands.add('openCenterLayoutSettings', () => {
  ...
  cy.captureLiveStep('center-layout-settings-panel-open');
});
