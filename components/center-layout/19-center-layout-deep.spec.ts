...
// =============================================================================
// 19 — Center Layout (layout-2) — Deep Component Test
//
...
//   templates/layouts/layout2.html (2 equal columns: one-half + one-half)
...
//
// Center is a LAYOUT (Layout section), not an assessment widget: it drops onto
// the Instructions-for-Students canvas (#desktop_view) via
// cy.dropComponent('Center'). Automated cases cover drop, column
// structure/count, drop-zone presence, the settings panel (Full Width), copy /
// delete, divider/outline, and security. Nested child-drop rules,
// Preview/Reader/Publish, media/tables/math/languages, multipart, persistence
// and CLP are deferred. See CENTER-LAYOUT-DEEP-AUTOMATION.md.
// =============================================================================
...
beforeEach(function () {
  ...
  cy.ensureCenterLayoutDeepSetup({ through: 'dropped', skipCanvasNav: true });
});

// ===========================================================================
// Suite A — Drop onto canvas (TC_11, TC_12)
// ===========================================================================
describe('19-A — Center Layout: Drop onto canvas (TC_11, TC_12)', () => {
  it('TC_DROP — Drop Center layout onto the Instructions canvas', function () {
    skipIfNoUrl(this);
    cy.ensureCenterLayoutDeepSetup({ through: 'dropped', skipCanvasNav: false });
    cy.get(`#desktop_view .sd-item ${LAYOUT_CANVAS.widget}`, { timeout: 20000 }).should('exist');
    cy.log('✅ Center layout dropped onto canvas');
  });
  ...
  it('TC_11/TC_12 — Layout is draggable and placed successfully in the step canvas', function () {
    ...
    cy.log('✅ TC_11/12: Center layout is draggable and placed on the active step page');
  });
});
...
// ===========================================================================
describe('19-B — Center Layout: Column structure (TC_27, TC_28, TC_57)', () => {
  ...
  it('TC_27b — Both columns are equal halves (one-half + one-half)', function () {
    skipIfNoUrl(this);
    cy.get(LAYOUT_CANVAS.columnNarrow).should('have.length', LAYOUT_RULES.columnCount);
    cy.log('✅ TC_27b: layout-2 column ratio is two equal one-half columns (source-verified)');
  });
...
// ===========================================================================
describe('19-C — Center Layout: Child drop zones (TC_13, TC_14, TC_19–TC_22)', () => {
...
// ===========================================================================
describe('19-D — Center Layout: Settings panel (TC_31)', () => {
  beforeEach(function () { skipIfNoUrl(this); cy.openCenterLayoutSettings(); });
...
// ===========================================================================
describe('19-E — Center Layout: Divider & outline (TC_53, TC_54)', () => {
...
// ===========================================================================
describe('19-F — Center Layout: Delete (TC_25, TC_59)', () => {
...
// Suite G — Security
// ===========================================================================
describe('19-G — Center Layout: Security', () => {
...
// ===========================================================================
describe('19-Z — Center Layout: Deferred (Bootstrap / nesting rules / Preview / Reader / Publish / lifecycle)', () => {
