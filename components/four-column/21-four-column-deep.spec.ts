...
// =============================================================================
// 21 — Four Column Layout (layout-4) — Deep Component Test
//
...
//   templates/layouts/layout4.html (4 equal columns: one-fourth ×4)
...
//
// Four Column is a LAYOUT (Layout section), not an assessment widget: it drops
// onto the Instructions-for-Students canvas (#desktop_view) via
// cy.dropComponent('Four Column'). Automated cases cover drop, column
// structure/count (4), drop-zone presence, the settings panel (Full Width),
// copy / delete, divider/outline, and security. Nested child-drop rules,
// Preview/Reader/Publish, media/tables/math/languages, multipart, persistence
// and CLP are deferred. See FOUR-COLUMN-DEEP-AUTOMATION.md.
// =============================================================================
...
beforeEach(function () {
  ...
  cy.ensureFourColumnDeepSetup({ through: 'dropped', skipCanvasNav: true });
});

// ===========================================================================
// Suite A — Drop onto canvas (TC_11, TC_12)
// ===========================================================================
describe('21-A — Four Column: Drop onto canvas (TC_11, TC_12)', () => {
  it('TC_DROP — Drop Four Column layout onto the Instructions canvas', function () {
    skipIfNoUrl(this);
    cy.ensureFourColumnDeepSetup({ through: 'dropped', skipCanvasNav: false });
    cy.get(`#desktop_view .sd-item ${LAYOUT_CANVAS.widget}`, { timeout: 20000 }).should('exist');
    cy.log('✅ Four Column layout dropped onto canvas');
  });
  ...
  it('TC_11/TC_12 — Layout is draggable and placed successfully in the step canvas', function () {
    ...
    cy.log('✅ TC_11/12: Four Column layout is draggable and placed on the active step page');
  });
});
...
// ===========================================================================
describe('21-B — Four Column: Column structure (TC_27, TC_28, TC_57)', () => {
  ...
  it('TC_27b — All four columns are equal quarters (one-fourth ×4)', function () {
    skipIfNoUrl(this);
    cy.get(LAYOUT_CANVAS.columnQuarter).should('have.length', LAYOUT_RULES.columnCount);
    cy.log('✅ TC_27b: layout-4 renders four equal one-fourth columns (source-verified)');
  });
...
// ===========================================================================
describe('21-C — Four Column: Child drop zones (TC_13, TC_14, TC_19–TC_22)', () => {
  ...
  it('TC_19–TC_22 — All four columns independently accept content (four drop zones)', function () {
    skipIfNoUrl(this);
    cy.get(LAYOUT_CANVAS.dropZones).should('have.length', LAYOUT_RULES.columnCount);
    cy.log('ℹ TC_19–22: all columns are independent drop zones; combination drops verified at runtime (deferred)');
  });
});
...
// ===========================================================================
describe('21-D — Four Column: Settings panel (TC_31)', () => {
  beforeEach(function () { skipIfNoUrl(this); cy.openFourColumnSettings(); });
...
// ===========================================================================
describe('21-E — Four Column: Divider & outline (TC_53, TC_54)', () => {
  it('TC_53/TC_54 — Vertical dividers render between the columns', function () {
    skipIfNoUrl(this);
    cy.get(LAYOUT_CANVAS.divider).should('have.length.greaterThan', 0);
    cy.log('ℹ TC_53/54: vertical dividers present in authoring; CLP outline / separator display verified in Reader (deferred)');
  });
});
...
// ===========================================================================
describe('21-F — Four Column: Delete (TC_25, TC_59)', () => {
...
// Suite G — Security
// ===========================================================================
describe('21-G — Four Column: Security', () => {
...
// ===========================================================================
describe('21-Z — Four Column: Deferred (Bootstrap / nesting rules / Preview / Reader / Publish / lifecycle)', () => {
