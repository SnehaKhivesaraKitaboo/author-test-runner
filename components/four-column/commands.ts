...
// =============================================================================
// Four Column Layout (layout-4) component commands — reusable workflows.
//
...
//   cy.captureLiveStep
//
// IMPORTANT — Four Column is a LAYOUT, not an assessment widget:
//   "layout-4" is NOT in support/module-setup.ts ASSESSMENT_DATA_TYPES, so the
...
//   cy.dropComponent('Four Column'), NOT into a Generic Step column.
// =============================================================================
...
declare global {
  ...
  namespace Cypress {
    interface Chainable<Subject = any> {
      ensureFourColumnDeepSetup(options?: {
        through?: 'canvas' | 'dropped';
        skipCanvasNav?: boolean;
      }): Chainable<void>;
      dropFourColumnIntoCanvas(): Chainable<void>;
      openFourColumnSettings(): Chainable<void>;
    }
  }
}
...
Cypress.Commands.add('ensureFourColumnDeepSetup', (options = {}) => {
  ...
  cy.dropFourColumnIntoCanvas();
});

Cypress.Commands.add('dropFourColumnIntoCanvas', () => {
  cy.navigateToCanvasPage();

  cy.get('#desktop_view').then(($desktop) => {
    const alreadyDropped = $desktop.find(`.sd-item ${LAYOUT_CANVAS.widget}`).length > 0;
    if (alreadyDropped) {
      cy.log('ℹ Four Column layout already present on canvas');
      return;
    }
  ...
  cy.captureLiveStep('widget-dropped-four-column');
});

Cypress.Commands.add('openFourColumnSettings', () => {
  ...
  cy.captureLiveStep('four-column-settings-panel-open');
});
